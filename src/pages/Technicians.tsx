...
const Technicians = () => {
  ...
  const openNew = () => { setEditId(null); setForm({ name: "", department: "", contact: "" }); setOpen(true); };
  const openEdit = (t: Tech) => { setEditId(t.id); setForm(t); setOpen(true); };

  const save = () => {
    if (!form.name) { toast.error("Name required / Nome obrigatório"); return; }
    if (editId) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={openNew}>
          <Plus />
          <BiLabel en="Add technician" pt="Adicionar técnico" size="small" />
        </Button>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead><BiLabel en="Department" pt="Departamento" size="table" /></TableHead>
          ...
          <TableBody>
            {list.length === 0 ? (
              <TableRow><TableCell colSpan={4} className="text-center py-10">
                <BiLabel en="No technicians" pt="Nenhum técnico" className="items-center" />
              </TableCell></TableRow>
            ) : list.map((t) => (
              <TableRow key={t.id}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <Label><BiLabel en="Department" pt="Departamento" size="small" /></Label>
